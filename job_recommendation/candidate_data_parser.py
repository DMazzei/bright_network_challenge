...
class CandidateDataParser:
    ...
    def extract_job_locations(self) -> JobLocationPreferences:
        ...
        as undesirable.
        """
        positive_location_matcher = self._set_matcher(key="LocationPositive", patterns=self.POSITIVE_LOCATION_PATTERNS)
        negative_location_matcher = self._set_matcher(key="LocationNegative", patterns=self.NEGATIVE_LOCATION_PATTERNS)
        ...
        return JobLocationPreferences(positive=positive_locations, negative=negative_locations)
    ...
    def _set_matcher(self, key: str, patterns: List) -> Matcher:
